...
def create_moving_point(fig, ax, renderer, ptx, pty, arendx, arendy, dir="botleft"):
    ...
    ar = ax.annotate(
        "",
        xy=(ptx, pty),
        xytext=(arendx, arendy),
        arrowprops=dict(
            arrowstyle="<-", shrinkA=0, shrinkB=0, zorder=4, mutation_scale=15
        ),
    )
    return pt, an, ar
...
def create_moving_line(
    fig, ax, renderer, pt1tup, pt2tup, movetup, dir="botleft", dir2=None
):
    ...
    conn_arrow = ax.annotate(
        "",
        xy=pt1tup,
        xytext=pt2tup,
        arrowprops=dict(arrowstyle="-", shrinkA=0, shrinkB=0, color="g", zorder=4),
    )

    move_pt1_arrow = ax.annotate(
        "",
        xy=pt1tup,
        xytext=(pt1tup[0] + movetup[0], pt1tup[1] + movetup[1]),
        arrowprops=dict(
            arrowstyle="<-",
            linestyle=":",
            shrinkA=0,
    ...
    move_pt2_arrow = ax.annotate(
        "",
        xy=pt2tup,
        xytext=(pt2tup[0] + movetup[0], pt2tup[1] + movetup[1]),
        arrowprops=dict(
            arrowstyle="<-",
            linestyle=":",
            shrinkA=0,
    ...
    end_pts_conn = ax.annotate(
        "",
        xy=(pt1tup[0] + movetup[0], pt1tup[1] + movetup[1]),
        xytext=(pt2tup[0] + movetup[0], pt2tup[1] + movetup[1]),
        arrowprops=dict(
            arrowstyle="-",
            shrinkA=0,
...
def create_moving_polygon(fig, ax, renderer, points, move, dir="botleft"):
    ...
    for p in points:
        ...
        ax.annotate(
            "",
            xy=(last[0] + move[0], last[1] + move[1]),
            xytext=(p[0] + move[0], p[1] + move[1]),
            arrowprops=dict(
                arrowstyle="-",
                shrinkA=0,
...
def create_still_segment(fig, ax, renderer, pt1tup, pt2tup, dir="botleft", dir2=None):
    ...
    segment_pts = ax.scatter(
        [pt1tup[0], pt2tup[0]], [pt1tup[1], pt2tup[1]], c="r", zorder=5
    )
    segment_arrow = ax.annotate(
        "",
        xy=pt1tup,
        xytext=pt2tup,
        arrowprops=dict(arrowstyle="-", shrinkA=0, shrinkB=0, color="b", zorder=4),
    )
