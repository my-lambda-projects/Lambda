...
class Solution(object):
    def helper(self, x, y, board, trie_node, result):
        if trie_node.value:
            result.add(
                trie_node.value
            )  # Look for other soln even if a soln is found. soln could a prefix of another soln.
        for x1, y1 in ((x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)):
            if (
                0 <= x1 < len(board)
                and 0 <= y1 < len(board[0])
                and board[x1][y1] != -1
                and trie_node.links[ord(board[x1][y1]) - ord("a")]
            ):
        ...
        for i in range(len(board)):
            for j in range(len(board[0])):
                if trie.root.links[ord(board[i][j]) - ord("a")]:
                    ...
                    self.helper(
                        i, j, board, trie.root.links[ord(ch) - ord("a")], result
                    )
                    board[i][j] = ch
