...
def create_suffix_array(s):
    ...
    for index, char in enumerate(s):
        ...
        suffix_obj.adjacent_rank = (
            ord(s[index + 1]) - ord("a") if (index + 1 < N) else -1
        )
    ...
    while no_char < 2 * N:
        ...
        for index in range(1, N):
            if (
                suffix_array[index].first_rank == prev_rank
                and suffix_array[index].adjacent_rank
                == suffix_array[index - 1].adjacent_rank
            ):
                suffix_array[index].first_rank = rank
            else:
                rank += 1
                prev_rank, suffix_array[index].first_rank = (
                    suffix_array[index].first_rank,
                    rank,
                )
...
def lcp_w_suffix_str(array, s):
    ...
    for index in range(N):
        ...
        while (
            index + maxLen < N
            and index_j + maxLen < N
            and s[index + maxLen] == s[index_j + maxLen]
        ):
            maxLen += 1
