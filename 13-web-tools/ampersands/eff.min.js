...
( function ( c, d ) {
  ...
  function q( a, b ) {
    return ( 0 == a ? "no" : String( a ).replace( v, "," ) ) + " " + b + ( 1 == a ? "" : "s" )
  }
  ...
  function h() {
    ...
    a.replace( w, function () {
      return ""
    } ).replace( x, function ( r, n ) {
      l.push( n );
      return ""
    } ).replace( y, function ( r, n ) {
      z.hasOwnProperty( n ) || b.push( r )
    } );
    var p = b.length;
    var m = l.length;
    k = "<a href=#" + f( a ) + ">#</a>";
    t.className = p || m ? "fail" :
      "";
    t.innerHTML = "<p>" + k + " Found " + q( p, "ambiguous ampersand" ) + ( p ? ": <ul><li><code>" + b.join( "</code><li><code>" ).replace( u, "&amp;" ) + "</code></ul>" : ". " ) + "<p>Found " + q( m, "character reference" ) + " that " + ( 1 == m ? "doesn\u2019t" : "don\u2019t" ) + " end with a semicolon" + ( m ? ": <ul><li><code>" + l.join( "</code><li><code>" ).replace( u, "&amp;" ) + "</code></ul>" : "." );
    g && ( g.ampersandText = a )
  }
  var e = d.getElementsByTagName( "textarea" )[ 0 ];
  d.getElementById( "characters" );
  var t = d.getElementById( "output" ),
    v = /(?=(?:\d{3})+$)(?!\b)/g,
    u = /&/g,
    y = /&([0-9a-zA-Z]+);/g,
    g = function () {
  ...
  g && ( g.ampersandText && ( e.value = g.ampersandText ), h() );
  c.onhashchange = function () {
    e.value = decodeURIComponent( location.hash.slice( 1 ) );
    h()
  };
  if ( location.hash ) c.onhashchange()
} )( this, document );
