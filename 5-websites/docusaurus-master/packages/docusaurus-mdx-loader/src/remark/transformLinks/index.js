...
const {
  getFileLoaderUtils
} = require( '@docusaurus/core/lib/webpack/utils' );
...
function toAssetRequireNode( {
  ...
  const href =
    `require('${inlineMarkdownLinkFileLoader}${relativeRequireAssetPath}').default`;
...
async function convertToAssetLinkIfNeeded( {
  ...
  if ( assetPath.startsWith( '@site/' ) ) {
    ...
    const fileSystemAssetPath = path.join( path.dirname( filePath ),
      assetPath );
    if ( await fs.exists( fileSystemAssetPath ) ) {
      toAssetLinkNode( fileSystemAssetPath );
    }
