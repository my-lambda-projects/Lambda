...
async function execute() {
  ...
  if ( fs.existsSync( join( CWD, '..', readMetadata.getDocsPath(),
      'assets' ) ) ) {
    fs.copySync(
  ...
  if ( fs.existsSync( join( CWD, 'blog', 'assets' ) ) ) {
    fs.copySync( join( CWD, 'blog', 'assets' ), join( buildDir, 'blog',
      'assets' ) );
  }
  ...
  files.forEach( ( file ) => {
    // Why normalize? In case we are on Windows.
    // Remember the nuance of glob: https://www.npmjs.com/package/glob#windows
    const targetFile = path.normalize( file ).replace( libStaticDir,
      buildDir );
    ...
    if ( file.match( /\.css$/ ) ) {
      ...
      Object.keys( siteConfig.colors ).forEach( ( key ) => {
        ...
        cssContent = cssContent.replace( new RegExp( `\\$${key}`,
          'g' ), color );
      } );
  ...
  files.forEach( ( file ) => {
    ...
    if (
      ...
      const targetFile = normalizedFile.replace( userStaticDir,
      buildDir );
      mkdirp.sync( path.dirname( targetFile ) );
      fs.copySync( normalizedFile, targetFile );
    }
  ...
  files.forEach( ( file ) => {
      ...
      if ( langParts.indexOf( 'en' ) !== -1 ) {
        ...
        for ( let i = 0; i < enabledLanguages.length; i++ ) {
          ...
          if (
            language === 'en' ||
            !fs.existsSync(
              normalizedFile.replace( `${sep}en${sep}`, sep + language +
                sep ),
            )
          ) {
            ...
            writeFileAndCreateFolder(
              ...
              targetFile.replace( `${sep}en${sep}`, sep + language +
                sep ),
              str,
            );
          }
    ...
    } else if ( siteConfig.wrapPagesHTML && normalizedFile.match(
        /\.html$/ ) ) {
