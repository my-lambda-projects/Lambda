...
function execute( port, host ) {
  ...
  function reloadMetadataBlog() {
    if ( fs.existsSync( join( __dirname, '..', 'core', 'MetadataBlog.js' ) ) ) {
      removeModuleAndChildrenFromCache( join( '..', 'core',
        'MetadataBlog.js' ) );
  ...
  app.get( routing.blog( siteConfig ), ( req, res, next ) => {
    ...
    removeModuleAndChildrenFromCache( join( '..', 'core',
      'BlogPageLayout.js' ) );
    const blogPages = blog.getPagesMarkup( MetadataBlog.length,
    siteConfig );
    const urlPath = req.path.toString().split( 'blog/' )[ 1 ];

    if ( urlPath === 'index.html' ) {
      res.send( blogPages[ '/index.html' ] );
    } else if ( urlPath.endsWith( '/index.html' ) && blogPages[
      urlPath ] ) {
  ...
  app.get( routing.page( siteConfig ), ( req, res, next ) => {
    ...
    if ( language && language !== 'en' ) {
      englishFile = englishFile.replace( sep + language + sep,
        `${sep}en${sep}` );
    }
  ...
  app.get( `${siteConfig.baseUrl}css/main.css`, ( req, res ) => {
    ...
    Object.keys( siteConfig.colors ).forEach( ( key ) => {
      ...
      cssContent = cssContent.replace( new RegExp( `\\$${key}`, 'g' ),
        color );
    } );
  ...
  app.use( siteConfig.baseUrl, express.static( join( __dirname, '..',
    'static' ) ) );
