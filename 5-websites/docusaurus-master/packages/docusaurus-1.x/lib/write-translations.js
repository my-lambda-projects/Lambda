...
if ( fs.existsSync( `${CWD}/data/custom-translation-strings.json` ) ) {
  customTranslations = _.merge( {},
    JSON.parse(
      fs.readFileSync( `${CWD}/data/custom-translation-strings.json`,
      'utf8' ),
    ),
    customTranslations,
  );
...
function parseJSXFile( file ) {
  try {
    ...
    throw new Error(
      `Babylon parsing failure for file=${file}: ${e.message}
...
function execute() {
  ...
  glob.sync( `${docsDir}/**` ).forEach( ( file ) => translateDoc( file,
    docsDir ) );
  glob
  ...
  Object.keys( sidebars ).forEach( ( sb ) => {
    ...
    Object.keys( categories ).forEach( ( category ) => {
      translations[ 'localized-strings' ].categories[ category ] =
        category;
    } );
  } );
