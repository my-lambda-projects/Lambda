...
files.forEach( ( file ) => {
  ...
  const res = metadataUtils.extractMetadata( fs.readFileSync( file,
    'utf8' ) );
...
if ( versionFallback.diffLatestSidebar() ) {
  ...
  const sidebar = JSON.parse( fs.readFileSync( `${CWD}/sidebars.json`,
    'utf8' ) );
  const versioned = {};
    ...
    Object.keys( categories ).forEach( ( category ) => {
      ...
      categoryItems.forEach( ( categoryItem ) => {
        ...
        if ( typeof categoryItem === 'object' ) {
          ...
          versionedCategoryItem =
            `version-${version}-${categoryItem}`;
        }
