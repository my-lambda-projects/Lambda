...
const {
  Joi,
  URISchema
} = require( '@docusaurus/utils-validation' );
...
const NavbarItemPosition = Joi.string().equal( 'left', 'right' ).default(
  'left' );
...
const ColorModeSchema = Joi.object( {
  ...
  disableSwitch: Joi.bool().default( DEFAULT_COLOR_MODE_CONFIG
    .disableSwitch ),
...
const ThemeConfigSchema = Joi.object( {
  ...
  navbar: Joi.object( {
    ...
    hideOnScroll: Joi.bool().default( DEFAULT_CONFIG.navbar
      .hideOnScroll ),
    // TODO temporary (@alpha-58)
  ...
  prism: Joi.object( {
      theme: Joi.object( {
        plain: Joi.alternatives().try( Joi.array(), Joi.object() )
          .required(),
        styles: Joi.alternatives().try( Joi.array(), Joi.object() )
          .required(),
      } ),
