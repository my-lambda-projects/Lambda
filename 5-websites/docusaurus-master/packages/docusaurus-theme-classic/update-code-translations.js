...
async function extractThemeCodeMessages() {
  ...
  const filesExtractedTranslations =
...
async function getCodeTranslationFiles() {
  ...
  const localesFiles = ( await globby( codeTranslationsDir ) ).filter(
    ( filepath ) =>
    path.extname( filepath ) === '.json' && !filepath.endsWith(
      'base.json' ),
  );
  return {
    baseFile,
...
${logKeys(unknownMessages)}` ),
  ...
  const newBaseMessagesDescriptions = Object.entries( newBaseMessages )
    .reduce(
      ( acc, [ key ] ) => {
        return {
          ...acc,
