...
( function () {
  ...
  var Game = function ( view, width, height, scale ) {
    ...
    this.terrain = [ {
      ...
      },
      {
        ...
        maxY: h - 250 * s,
        bounce: true,
        labels: [ "building0", "building1", "building2" ]
      },
      {
        speed: 3,
        minX: 809 - 2,
      ...
      },
      {
        ...
        maxY: h - 200 * s,
        bounce: true,
        labels: [ "midDeco0", "midDeco1", "midDeco2", "midDeco3" ]
      },
      {
        speed: 4,
        minX: 100,
      ...
      },
      {
        ...
        type: 1,
        x: 2000,
        labels: [ "trap0", "trap1", "trap2", "enemy0Idle", "enemy1Idle" ]
      },
    ];
  ...
  p.init = function ( spriteSheet ) {
    ...
    if ( !this._inited ) {
      ...
      for ( i = 0, l = terrain.length; i < l; i++ ) {
        var o = this.view.addChild( new c.Container( spriteSheet ) );
    ...
    c.Tween.get( this.stats ).wait( 1000 ).to( {
      x: 40,
      alpha: 1
    }, 1000, c.Ease.easeOut );
    ...
    this.hero.set( {
      x: -this.hero.getBounds().width,
      y: this.height - 80 * s,
      right: 1000
    } )
    c.Tween.get( this.hero ).wait( 1000 ).to( {
      x: 160 * s
    }, 3000, c.Ease.get( 1 ) );

    this.shot = this.getSprite( "bulletIdle", 0, false, null, 0 ).set( {
      visible: false,
      y: this.hero.y - 130 * s
    } );
    this.shot.stop();
    ...
    this.speed = 200 * s;
    for ( i = 0; i < this.width * 1.1 / this.speed; i++ ) {
      this.updateTerrain();
    }
    this.speed = 5;
  ...
  p.handleKeyDown = function ( evt ) {
    if ( this.dead || this.hero.currentAnimation != "run" ) {
      return;
    }
    if ( evt.keyCode == 38 ) {
  ...
  p.updateTerrain = function ( enemies, t ) {
    ...
    for ( i = this.sprites.length - 1; i >= 0; i-- ) {
      ...
      if ( !this.dead && sprite.x < bounds.x - bounds.width ) {
        this.reclaimSprite( sprite, i );
      } else if ( sprite.type ) {
  ...
  p.getSprite = function ( label, speed, bounce, containerIndex, type ) {
    ...
    if ( containerIndex != null ) {
      this.terrainContainers[ containerIndex ].addChild( sprite );
    }
