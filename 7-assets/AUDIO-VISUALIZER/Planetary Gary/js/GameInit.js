...
( function () {
  ...
  function init() {
    ...
    queue.loadManifest( [ {
        src: "assets/PlanetaryGary_art.js",
        id: "art"
      },
      {
        src: "js/Game.js"
  ...
  function handleFileLoad( evt ) {
    ...
    if ( o.id == "music" && o.type == c.LoadQueue.SOUND ) {
      ...
      music = c.Sound.play( "music", {
        loop: -1
      } );
      setMusicVolume( 1, 10000 );
      setupMusicFX( music );
    } else if ( o.id == "art" ) {
      var comp = AdobeAn.getComposition( "66509341DAAA44439E6E47782F0AE206" );
      lib = comp.getLibrary();
  ...
  function playSoundFX() {
    ...
    createjs.Tween.get( delayNode.delayTime, {
        override: true
      } )
      .to( {
        value: 1
        ...
        value: 0
      }, 2200, createjs.Ease.bounceInOut );
  }
  ...
  function handleBuildComplete() {
    ...
    for ( n in o ) {
      spriteSheet.getAnimation( n ).next = o[ n ];
    }

    // if the user has already hit start, then start the game:
    if ( builder.startGame ) {
      startGame();
    }
    builder = null;
