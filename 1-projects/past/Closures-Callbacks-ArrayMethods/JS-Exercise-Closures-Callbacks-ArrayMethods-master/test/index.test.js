...
describe("processFirstItem()", () => {
  it("works as specified", () => {
    expect(processFirstItem(["bar", "baz"], (str) => str + str)).to.equal(
      "barbar"
    );
...
describe("processLastItem()", () => {
  it("works as specified", () => {
    expect(processLastItem(["bar", "baz"], (str) => str + str)).to.equal(
      "bazbaz"
    );
...
describe("getFullNames()", () => {
  it("returns an array of strings", () => {
    expect(getFullNames(runners).every((r) => typeof r === "string")).to.equal(
      true
    );
  });
...
describe("firstNamesAllCaps()", () => {
  it("returns an array of strings", () => {
    expect(
      firstNamesAllCaps(runners).every((r) => typeof r === "string")
    ).to.equal(true);
  });
  it("returns an array of the same length as the original array", () => {
...
describe("getRunnersByTShirtSize()", () => {
  it("returns an array of runners of a given shirt size", () => {
    expect(
      getRunnersByTShirtSize(runners, "S").every((r) => r.shirt_size === "S")
    ).to.equal(true);
  });
...
describe("tallyUpDonations() [STRETCH TASK]", () => {
  ...
  it("returns the expected amount for a small dataset", () => {
    expect(
      tallyUpDonations([{ donation: 1 }, { donation: 2 }, { donation: 3 }])
    ).to.equal(6);
  });
