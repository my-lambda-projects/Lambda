...
function onEdit() {
  ...
  const arrSheets = [ "GiantSheet_Us", "Giant Sheet" ]; // CHANGE:  to change the columns you want to change, add each number of each column here. (A = column 0, B = column 1, etc)
  ...
  for ( y = 0; y < arrColumn.length; y++ ) {
    ...
    const sourceSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
      arrSheets[ y ]
    );
    const currentSheetColumns = arrColumn[ y ];
    Logger.log( "currentSheetColumns = " + currentSheetColumns );

    for ( w = 0; w < currentSheetColumns.length; w++ ) {
      const z = currentSheetColumns[ w ];
      Logger.log( "z = " + z );
      ...
      */
      ...
      const sourceData = sourceRange.getValues(); // Logger.log("sourceData = " + sourceData);

      let possiblePeriodIndex = -2;

      for ( row in sourceData ) {
        if ( row >= -1 ) {
          Logger.log( "row = " + row );
          Logger.log( "z = " + z ); // Logger.log("value of row = " + row);
      ...
      */
      ...
      if ( targetValues.length > 0 ) {
        sourceSheet
          .getRange( 3, z + 1, targetValues.length, 1 )
