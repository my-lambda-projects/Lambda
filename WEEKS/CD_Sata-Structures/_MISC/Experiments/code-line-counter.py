...
def clean_data(window):
    ...
    code_stats = {
        "lines": len(clean_code),
        "char_per_line": char_per_line,
        "count": sum(char_cnt),
        "mean": stats.mean(char_cnt),
        "median": stats.median(char_cnt),
        "pstdev": stats.pstdev(char_cnt),
        "min": min(char_cnt),
        "max": max(char_cnt),
    }
...
def click_file(window):
    ...
    filename = sg.popup_get_file(
        "Select a file containing Python code:", title="Code Counter"
    )
    if filename is None:
        return
...
def stat(text, width=10, relief=None, justification="left", key=None):
    return sg.Text(
        text, size=(width, 1), relief=relief, justification=justification, key=key
    )
...
def main():
    ...
    tab1 = sg.Tab(
        ...
        [
            [
                sg.Multiline(
                    key="-MLINE RAW-", size=(90, 30), pad=(0, 0), font="Courier 12"
                )
            ]
        ],
    ...
    tab2 = sg.Tab(
        ...
        [
            [
                sg.Multiline(
                    key="-MLINE CLEAN-", size=(90, 30), pad=(0, 0), font="Courier 12"
                )
            ]
        ],
    ...
    stat_col = sg.Column(
        [
            [
                stat("Lines of code"),
                stat(0, 8, "sunken", "right", "LINES"),
                stat("Total chars"),
                stat(0, 8, "sunken", "right", "CHARS"),
            ],
            [
                stat("Chars per line"),
                stat(0, 8, "sunken", "right", "CPL"),
                stat("Mean"),
                stat(0, 8, "sunken", "right", "MEAN"),
            ],
            [
                stat("Median"),
                stat(0, 8, "sunken", "right", "MEDIAN"),
                stat("PStDev"),
                stat(0, 8, "sunken", "right", "PSTDEV"),
            ],
            [
                stat("Max"),
                stat(0, 8, "sunken", "right", "MAX"),
                stat("Min"),
                stat(0, 8, "sunken", "right", "MIN"),
            ],
        ],
        pad=(5, 10),
    )
    ...
    layout = [
        ...
        [
            ...
            sg.T(
                "Original code written by THE Israel Dryer (github.com/israel-dryer)",
                pad=(0, 0),
            ),
        ],
        [sg.T("Python ver " + sg.sys.version, font="Default 8", pad=(0, 0))],
    ]
    ...
    [
        ...
        for elem in ["-MLINE RAW-", "-MLINE CLEAN-", "-TAB GROUP-"]
    ]
