...
class PearsonLaTeXWriter(writers.Writer):
    ...
    def __init__(self, builder):
        ...
        self.translator_class = self.builder.translator_class or PearsonLaTeXTranslator
...
class PearsonLaTeXTranslator(nodes.NodeVisitor):
    ...
    def astext(self):
        return HEADER % self.elements + u"".join(self.body)
    ...
    def visit_title(self, node):
        ...
        if isinstance(parent, addnodes.seealso):
            ...
            if node.traverse(nodes.image):
                short = "[%s]" % u" ".join(clean_astext(node).split()).translate(
                    tex_escape_map
                )
    ...
    def depart_table(self, node):
        ...
        if self.table.caption:
            self.body.append(
                self.hypertarget(
                    "table:%s" % self.table.caption[0], withdoc=False, anchor=False
                )
            )
    ...
    def visit_figure(self, node):
        ...
        for c in node.children:
            if isinstance(c, nodes.caption):
                ...
                ids += self.hypertarget(
                    "figure:%s" % caption, withdoc=False, anchor=False
                )
                break
        self.restrict_footnote(node)
    ...
    def visit_literal_block(self, node):
        ...
        if node.rawsource != node.astext():
            ...
            if caption:
                ...
                self.literal_block_caption = (
                    []
                )  # clear the caption so it isn't repeated
            if ids:
