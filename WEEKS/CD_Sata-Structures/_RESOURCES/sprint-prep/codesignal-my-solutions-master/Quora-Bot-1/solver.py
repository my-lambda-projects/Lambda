...
def mostViewedWriters(topicIds, answerIds, views):
    ...
    uniq_topic_ids = set(
        [topicIds[i][j] for i in range(len(topicIds)) for j in range(len(topicIds[i]))]
    )
    ...
    for u in uniq_topic_ids:
        ...
        cur_answer_ids = [
            tmp[i][j] for i in range(len(tmp)) for j in range(len(tmp[i]))
        ]

        cur_user_id = [
            views[x][1] for x in range(len(views)) if views[x][0] in cur_answer_ids
        ]
        cur_views = [
            views[x][2] for x in range(len(views)) if views[x][0] in cur_answer_ids
        ]
        ...
        for (user, view) in zip(cur_user_id, cur_views):
            if u not in final_dict:
                ...
                id_to_add_views = [
                    x for x in range(len(final_dict[u])) if final_dict[u][x][0] == user
                ]
    ...
    for k in sorted(uniq_topic_ids):
        if k not in final_dict:
            ...
            sorted_sublist = sorted(
                sorted_sublist1, key=operator.itemgetter(1), reverse=True
            )
