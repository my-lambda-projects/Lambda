    ...
    def find_path(self, graph, start, end, heuristic_fn):
        ...
        while len(_open) > 0:
            ...
            for neighbor in neighbors:
                ...
                cost_start_to_neighbor = current_dict[
                    "dist_start_to_here"
                ...
                if neighbor_from_lookup is not None:
                    ...
                    old_dist_start_to_neighbor = neighbor_from_lookup[
                        "dist_start_to_here"
                    ]

                    if cost_start_to_neighbor < old_dist_start_to_neighbor:
                        pred_dist_neighbor_to_end = neighbor_from_lookup[
                            "pred_dist_here_to_end"
                        ]
                        pred_total_dist_through_neighbor_to_end = (
                            cost_start_to_neighbor + pred_dist_neighbor_to_end
                        )
                ...
                heapq.heappush(
                    _open, (pred_total_dist_through_neighbor_to_end, counter, neighbor)
                )
                _open_lookup[neighbor] = {
                    "vertex": neighbor,
                    "dist_start_to_here": cost_start_to_neighbor,
...
class BiDirectionalAStar(object):
    ...
    def find_path(self, graph, start, end, heuristic_fn):
        ...
        while len(open_by_start) > 0 and len(open_by_end) > 0:
            result = self._evaluate_from_start(
                graph,
                start,
                end,
                heuristic_fn,
            ...
            result = self._evaluate_from_end(
                graph,
                start,
                end,
                heuristic_fn,
    ...
    def _evaluate_from_start(
        ...
        for neighbor in neighbors:
            ...
            if (
                neighbor_dict is not None
                and neighbor_dict["source"] is self.NodeSource.BY_END
            ):
                return self.reverse_path(current_dict, neighbor_dict)

            dist_to_neighb_through_curr_from_start = current_dict[
                "dist_start_to_here"
            ] + graph.get_edge_weight(current_vertex, neighbor)
            ...
            if neighbor_dict is not None:
                ...
                if (
                    neighbor_dict["dist_start_to_here"]
                    <= dist_to_neighb_through_curr_from_start
                ):
                    continue

                pred_dist_neighbor_to_end = neighbor_dict["pred_dist_here_to_end"]
                pred_total_dist_through_neighbor = (
                    dist_to_neighb_through_curr_from_start + pred_dist_neighbor_to_end
                )
                open_lookup[neighbor] = {
                    "vertex": neighbor,
                    "parent": current_dict,
                    "source": self.NodeSource.BY_START,
            ...
            heapq.heappush(
                open_by_start,
                (pred_total_dist_through_neighbor, counter_arr[0], neighbor),
            )
            counter_arr[0] += 1

    def _evaluate_from_end(
        self,
        graph,
        start,
        end,
        ...
        for neighbor in neighbors:
            ...
            if (
                neighbor_dict is not None
                and neighbor_dict["source"] is self.NodeSource.BY_START
            ):
                return self.reverse_path(neighbor_dict, current_dict)

            dist_to_neighb_through_curr_from_end = current_dict[
                "dist_end_to_here"
            ] + graph.get_edge_weight(current_vertex, neighbor)
            ...
            if neighbor_dict is not None:
                ...
                if (
                    neighbor_dict["dist_end_to_here"]
                    <= dist_to_neighb_through_curr_from_end
                ):
                    continue

                pred_dist_neighbor_to_start = neighbor_dict["pred_dist_here_to_start"]
                pred_total_dist_through_neighbor = (
                    dist_to_neighb_through_curr_from_end + pred_dist_neighbor_to_start
                )
                open_lookup[neighbor] = {
                    "vertex": neighbor,
                    "parent": current_dict,
