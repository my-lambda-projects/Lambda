...
def test_collinear(pt1, pt2, pt3):
    ...
    Cy = pt3[1]
    return math.isclose(
        Ax * (By - Cy) + Bx * (Cy - Ay) + Cx * (Ay - By), 0, abs_tol=1e-07
    )
...
def gen_rand_poly_verts(minx, maxx, miny, maxy, minrad, maxrad):
    ...
    while True:
        newpt = (
            centerx + round(math.cos(curr_angle * math.pi / 180) * radx),
            centery + round(math.sin(curr_angle * math.pi / 180) * rady),
        )
        ...
        if not found:
            collinear = (
                False
                if len(result) < 3
                else test_collinear(result[-2], result[-1], newpt)
            )
...
def gen_problem():
    ...
    return (
        ...
        movingpoly,
        movevec,
        (math.floor(xmin) - 1, math.ceil(xmax) + 1),
        (math.floor(ymin) - 1, math.ceil(ymax) + 1),
    )
...
def save_problem(stillpoly, movingpoly, movevec, xlim, ylim):
    ...
    print(
        "    create_moving_polygon(fig, ax, renderer, {}, {})".format(mpolystr, mvecstr)
    )
