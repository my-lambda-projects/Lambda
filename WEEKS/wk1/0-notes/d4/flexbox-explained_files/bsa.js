var _bsap = {
  ...
  exec: function () {
    this.prem = {};
    this.callback =
      typeof BSACallback === "function" ? BSACallback : function () {};
    for (
      var cl = function (cl) {
          for (
        ...
        i = 0;
      i < bs.length &&
      (id = bs[i].getAttribute("id")) &&
      (rid = id.split("_")[1]) &&
      (pk =
        bs[i].getAttribute("rel") ||
        ((pk = p.exec(bs[i].className)) ? pk[1] : "")) &&
      (bs[i].className = "bsap_" + rid + " bsap");
      i++
    )
  ...
  deploy: function () {
    for (
      var zi = 0, wi, z, a, b, db;
      zi < this.jz.length && (z = this.jz[zi]) && (wi = 1);
      zi++
    )
      while (
        ++wi < 10 &&
  ...
  zone: function (b, z, bsa) {
    ...
    function flushWrite(which) {
      if (bsa.writes[which]) {
        var el = document.getElementById(which),
          writer = el.parentNode.insertBefore(
    ...
    function stepQueue(which) {
      if (
        bsa.scriptqueue.length > 0 &&
        bsa.scriptqueue[0].which == which &&
        !bsa.scriptqueue[0].run
      )
        bsa.scriptqueue.shift();
      runQueue();
    }
    function runQueue() {
      ...
      bsa.scriptqueue.push({
        ...
        run: (function (id, sc) {
          return function () {
            ...
            bsa.writescript = sc.parentNode.insertBefore(
    ...
    function findScripts(t) {
      for (var i = 0; i < t.childNodes.length; i++)
        t.childNodes[i].nodeName.toLowerCase() == "script" &&
        t.childNodes[i].className != "ignoreme"
          ? gotScript(t.childNodes[i])
          : findScripts(t.childNodes[i]);
    }
    ...
    bsa.filter(
      ...
      function (f) {
        ...
        for (i = 0; zs && i < f.length; i++)
          zf = zf.concat(
            typeof zs[f[i].toLowerCase()] == "object"
              ? zs[f[i].toLowerCase()].ads
              : []
          );
        ...
        var empty = 0,
          ...
          css = function (x) {
            ...
            newel.type = "text/css";
            newel.id = "bsa_css";
            var ie6 = newel.styleSheet
                ? typeof newel.styleSheet.cssText == "unknown"
                : false,
              el = !ie6 ? document.getElementById("bsa_css") || newel : newel;
            el.styleSheet
              ? ie6
                ? (el.styleSheet.cssText = x)
                : (el.styleSheet.cssText += x)
        ...
        if (z.premium) {
          ...
          var srv =
            z.premium +
            (bsa.prem[z.premium] > 1 ? "_" + bsa.prem[z.premium] : "");
          if (!bsa.prem[b.id]) {
            bsa.prem[b.id] = true;
            bsa.prem[z.premium]++;
        ...
        if (zf && zf.ads)
          for (empty = 100, i = 0; i < zf.ads.length; i++)
            empty -= zf.ads[i].per / (z.model == 1 ? z.nads : 1);
        if ((z.noempties && z.noempties == 1) || bsa.oshow) empty = 0;
        if (t <= 1) {
          var ah =
              (typeof ShowAdHereBanner === "object"
                ? ShowAdHereBanner[z.id]
                : z.showadhere) > 0,
            ra =
              (typeof RepeatAll === "object" ? RepeatAll[z.id] : z.repeathere) >
              0;
        }
        if (tf == 0) {
          if (typeof ShowAdHereBanner === "undefined" && !z.nostyle) {
            var bs = z.bannerstyles,
              sc =
                ...
                z.id +
                "{width:" +
                (z.vertical > 0 ? w + "px" : "100%") +
                ";display:block}div.bsap_" +
                z.id +
                " a{width:" +
                w +
                "px}div.bsap_" +
                z.id +
                " a img{padding:0}div.bsap_" +
                z.id +
                " a em{font-style:normal}";
            for (i = 0; i < bs.length; i++)
              sc += "div.bsap_" + z.id + " " + bs[i];
            if (w < 100)
              sc +=
                "div.bsap_" +
                z.id +
                " a em{display:block;text-indent:-9000px}div.bsap_" +
                z.id +
                " a{height:" +
                h +
                ";line-height:0}div.bsap_" +
                z.id +
                " a.adhere{font-size:0}";
            sc +=
              "div.bsap_" +
              z.id +
              " a{line-height:100%}div.bsap_" +
              z.id +
              " a.adhere{width:" +
              w +
              "px;height:" +
            ...
            if (z.model == 1)
              sc +=
                w == 728
                  ? "div.bsap_" +
                    z.id +
                    "{line-height:9px}div.bsap_" +
                    z.id +
                    " .bsap_adhere2,div.bsap_" +
                    z.id +
                    " iframe{float:left}div.bsap_" +
                    z.id +
                    " .bsap_adhere2 a{height:90px;width:20px;background:url(//s3.buysellads.com/ac/ah20x90_1.gif) no-repeat 0 0;text-indent:-9999px}"
                  : "div.bsap_" +
                    z.id +
                    "{line-height:9px}div.bsap_" +
                    z.id +
                    " .bsap_adhere a{height:19px;width:" +
                    (w - 2) +
                    "px;font-size:10px;background:#f1f1f1;border:1px solid #e1e1e1;border-top:none;border-bottom-left-radius:4px;-moz-border-radius-bottomleft:4px;-webkit-border-bottom-left-radius:4px;border-bottom-right-radius:4px;-moz-border-radius-bottomright:4px;-webkit-border-bottom-right-radius:4px;text-shadow:1px 1px 0 #fff;line-height:16px}.bsap_backfillframe{border:0}";
            css(sc);
          }
          for (
            i = 0, j = 0;
            i < c.length && (a = bsa.getads(c[i], 1)[0]);
            i++,
              Math.random() * 100 <
                (z.model == 1 && z.nads > c.length
                  ? 100 - c[i - 1].per
                  : empty) ||
                (j++,
                (o += a.rawframe
                  ...
                  : a.flash && bsa.hasFlash()
                  ? '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="' +
                    z.width +
                    '" height="' +
                    z.height +
                    '" id="bsa_flash"><param name="movie" value="' +
                    a.flash +
                    '"/><param name="menu" value="false"/><param name="quality" value="high"/><param name="wmode" value="opaque"/><param name="allowScriptAccess" value="always"/><param name="FlashVars" value="clickTag=' +
                    encodeURIComponent(
                      bsa.tracker(
                        "click.go",
                        a.id,
                    ...
                    '" height="' +
                    z.height +
                    '" quality="high" wmode="opaque" menu="false" allowScriptAccess="always" swliveconnect="true" flashvars="clickTag=' +
                    encodeURIComponent(
                      bsa.tracker(
                        "click.go",
                        a.id,
                  ...
                  : "<a " +
                    bsa.link(a.id, z.id, a.link.replace("[timestamp]", ts)) +
                    ' class="ad' +
                    j +
                    " " +
                    (j % 2 === 0 ? "even" : "odd") +
          ...
          if (z.cpmbackfill && j == 0) {
            for (
              var r = Math.random(), bn = 0;
              bn < z.cpmbackfill.length &&
              z.cpmbackfill[bn] &&
              z.cpmbackfill[bn][0] < r;
              r -= z.cpmbackfill[bn++][0]
            ) {}
            if (
              z.cpmbackfill[bn] &&
              z.cpmbackfill[bn][0] &&
              z.cpmbackfill[bn][1].length > 0
            ) {
              if (z.cpmbackfill[bn][1].substring(0, 2) != "//") {
                ...
                o +=
                  ...
                  '_frame" src="' +
                  z.cpmbackfill[bn][1] +
                  '" frameborder="0" class="bsap_backfillframe" scrolling="no"></iframe>';
              backfilled = true;
            }
          }
          for (
            i = j;
            !backfilled && i < z.nads && ah && i < (ra ? z.nads : j + 1);
            i++,
              o +=
                bsa.callback(z.id, i - 1, z.nads) ||
                '<a href="https://www.buysellads.com/buy/detail/' +
                  z.siteid +
                  "/zone/" +
                  z.id +
                  "?utm_source=site_" +
                  z.siteid +
                  "&utm_medium=website&utm_campaign=adhere&utm_content=zone_" +
                  z.id +
                  '" title="Advertise Here" class="adhere ad' +
                  i +
                  " " +
                  (i % 2 === 0 ? "even" : "odd") +
                  '" target="_blank">Advertise Here</a>'
          ) {}
          if (z.model == 1 && ah && (j == z.nads || backfilled))
            o +=
              ...
              "?utm_source=site_" +
              z.siteid +
              "&utm_medium=website&utm_campaign=cpmadhere&utm_content=zone_" +
              z.id +
              '" target="_blank">advertise here</a></div>';
        } else if (tf == 1) {
          ((z.model == 1 && Math.random() * 100 * z.nads < (empty || 0)) ||
            c.length == 0) &&
            ah &&
            (c = [
              {
                id: -i,
                link:
                  "https://www.buysellads.com/buy/detail/" +
                  z.siteid +
                  "/zone/" +
                  z.id,
          ...
          for (
            ...
            i++,
              ...
              (o +=
                ...
                '"><a ' +
                bsa.link(a.id, z.id, a.link.replace("[timestamp]", ts)) +
                ' target="_blank"><span class="bsa_it_i"><img src="' +
                a.img.replace("[timestamp]", ts) +
                '" width="' +
                z.width +
                '" height="' +
                ...
                a.alt +
                '" /></span></a><a ' +
                bsa.link(a.id, z.id, a.link) +
                ' target="_blank"><span class="bsa_it_t">' +
                a.alt +
                '</span><span class="bsa_it_d">' +
                a.text +
                '</span></a><div style="clear:both"></div></div>'),
              (t += a.id + ";"))
          ) {}
          if (z.cpmbackfill && j == 0) {
            for (
          ...
          for (
            i = j;
            !backfilled && typeof bsa.callback == "function" && i < z.nads;
            i++, o += bsa.callback(z.id, i - 1, z.nads) || ""
          ) {}
          if (c.length > 0)
            o +=
              '<span class="bsa_it_p"><a href="https://www.buysellads.com/buy/detail/' +
              z.siteid +
              "/zone/" +
              z.id +
              "?utm_source=site_" +
              z.siteid +
              "&utm_medium=website&utm_campaign=imagetext&utm_content=zone_" +
              z.id +
              '" target="_blank">ads by BSA</a>' +
              (z.carrousel
                ? '<br/><a href="#" class="prevad" onclick="_bsap.inv[' +
                  z.id +
                  ']()">&larr;</a> <a href="#" class="nextad" onclick="_bsap.inv[' +
                  z.id +
                  ']()">&rarr;</a>'
                : "") +
              "</span></div>";
          ...
          for (
            ...
            i++,
              o +=
                ...
                '"><a ' +
                bsa.link(a.id, z.id, a.link) +
                ' target="_blank"><div class="bwr"><em class="bt">' +
                a.title +
                '</em><em class="bd">' +
                a.text +
                "</em></div></a></li>",
          ...
          for (
            i = j;
            typeof bsa.callback == "function" && i < z.nads;
            i++, o += bsa.callback(z.id, i - 1, z.nads) || ""
          ) {}
          o += "</ul>";
          if (c.length > 0)
            o +=
              '<div class="bsa_idb"><span class="bsa_idl"><a href="https://www.buysellads.com/buy/detail/' +
              z.siteid +
              "?utm_source=site_" +
              z.siteid +
              "&utm_medium=website&utm_campaign=textdesc&utm_content=zone_" +
              z.id +
              '" target="_blank">ads by <em>BSA</em></a></span></div></div></div>';
        } else if (tf == 4) {
          ...
          if (
            a &&
            Math.random() * 100 >
              (z.model == 1 && z.nads > c.length ? 100 - c[0].per : empty)
          ) {
            b.style.backgroundImage =
              "url(" + a.img.replace("[timestamp]", ts) + ")";
            ...
            b.onclick = function (e) {
              ...
              if (target.getAttribute("id") == "bsap_" + z.id) {
                ...
                window.location = bsa.tracker(
                  "click.go",
                  a.id,
                  z.id,
                  "",
                  "",
                  "&link=" + url.textContent.replace("[timestamp]", ts)
                );
              }
            };
            t += a.id;
        ...
        if (o && !b.innerHTML.length) {
          ...
          if (exec.length)
            for (i = 0; i < exec.length; i++)
              setInnerHtmlAndExec(
                document.getElementById(exec[i][0]),
                exec[i][1]
              );
        }
  ...
  getads: function (d, n) {
    ...
    while (
      ret.length < n &&
      i++ < n * 100 &&
      (c = a[b[Math.floor(Math.random() * b.length)]])
    )
      if (n === 1) return [c];
      else if (!got[c.id]) {
        got[c.id] = 1;
  ...
  filter: function (fs, fn, noto) {
    ...
    if (
      fs.os &&
      fs.os[
        (os = (/(win|mac|linux|iphone|blackberry|pike)/.exec(
    ...
    if (fs.geo) {
      ...
      IAm = function (y) {
        if (y !== "?") {
          ...
          document.cookie =
            "country=" + y + "; expires=" + d.toGMTString() + "; path=/";
        }
        if (IAmGot.length > 0) {
          s.push(y);
      ...
      if (i >= 0) IAm(c.substring(i + 7 + 1).split(";")[0]);
        ...
        e.src =
          document.location.protocol + "//stats.buysellads.com/country.php";
  ...
  link: function (b, z, l) {
    return (
      'href="' +
      this.tracker("click.go", b, z, "", "", "&link=" + l) +
  ...
  br: function () {
    ...
    return [
      (/(camino|chrome|firefox|opera|msie|safari)/.exec(i) || ["", "?"])[1],
      parseFloat(
        (/(camino|chrome|firefox|opera|msie|safari)(\/|\s)([a-z0-9\.\+]*?)(\;|dev|rel|\s|$)/.exec(
          i
        ) || [0, 0, 0, 0])[3],
        10
      ) || 0,
      (/(win|mac|linux|iphone|blackberry|pike)/.exec(p.toLowerCase()) || [
        "?",
      ])[0],
    ];
  },
  gen: function (w, e) {
    ...
    if (i >= 0) return c.substring(i + w.length + 1).split(";")[0];
      ...
      document.cookie =
        w +
        "=" +
        (nd + Math.random().toString().substr(2, 7)) +
        "; expires=" +
        d.toGMTString() +
        "; path=/";
      return -1;
