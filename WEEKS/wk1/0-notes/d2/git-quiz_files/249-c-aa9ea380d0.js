(window["canvasWebpackJsonp"] = window["canvasWebpackJsonp"] || []).push([
  [249],
  {
      ...
      const l = {
        async loadBindings() {
          await Promise.all([
            ...
            s.e(118),
            s.e(648),
          ]).then(s.bind(null, "T3Mz"));
        },
        async ready() {
          await this.loadBindings();
          const t = a()("#rubrics.rubric_dialog");
          t.dialog({
            title: o.t("titles.details", "Assignment Rubric Details"),
            width: 600,
            resizable: true,
          });
        },
        ...
        async replaceLoadingDialog(t, e) {
          await this.loadBindings();
          a()("body").append(t);
          e.dialog("close");
          e.remove();
        ...
        async createRubricDialog(t, e) {
          const s = a()("#rubrics.rubric_dialog");
          if (s.length) await l.ready();
          else {
            const s = l.buildLoadingDialog();
            if (void 0 === e || null === e) {
              const e = await a.a.get(t);
              await l.replaceLoadingDialog(e, s);
            } else await l.replaceLoadingDialog(e, s);
          }
        },
      };
      a()(document).ready(function () {
        a()(".show_rubric_link").click(function (t) {
          t.preventDefault();
    ...
    PHBv: function (t, e, s) {
      ...
      class z extends r.a.Model {
        initialize() {
          ...
          this.sisIntegrationSettingsEnabled =
            this.sisIntegrationSettingsEnabled.bind(this);
          this.maxNameLength = this.maxNameLength.bind(this);
          this.maxNameLengthRequiredForAccount =
            this.maxNameLengthRequiredForAccount.bind(this);
          this.dueDateRequiredForAccount =
            this.dueDateRequiredForAccount.bind(this);
        ...
        initId() {
          this.id = this.isQuizzesNext()
            ? "assignment_" + this.get("id")
            : this.get("id");
        }
        initAssignment() {
          this.attributes.assignment &&
            this.set("assignment", new p["a"](this.attributes.assignment));
          return this.set("post_to_sis_enabled", this.postToSISEnabled());
        }
        initAssignmentOverrides() {
          if (this.attributes.assignment_overrides) {
            const t = new _["a"](this.attributes.assignment_overrides);
            return this.set("assignment_overrides", t, {
              silent: true,
            });
          }
        }
        initUrls() {
          if (this.get("html_url")) {
            this.set(
              "base_url",
              this.get("html_url").replace(/(quizzes|assignments)\/\d+/, "$1")
            );
        ...
        initTitleLabel() {
          return this.set(
            "title_label",
            this.get("title") || this.get("readable_type")
          );
        }
        initUnpublishable() {
          if (false === this.get("can_unpublish") && this.get("published"))
            return this.set("unpublishable", false);
        }
        initQuestionsCount() {
          const t = this.get("question_count");
          t &&
            this.set(
              "question_count_label",
              g.t("question_count", "Question", {
                count: t,
              })
            );
        }
        initPointsCount() {
          const t = this.get("points_possible");
          let e = "";
          t &&
            t > 0 &&
            !this.isUngradedSurvey() &&
            (e = Number.isInteger(t)
              ? g.t("assignment_points_possible", "pt", {
                  count: t,
                })
              : g.t("%{points} pts", {
                  points: g.n(t),
                }));
          return this.set("possible_points_label", e);
        }
        isQuizzesNext() {
          return "quizzes.next" === this.get("quiz_type");
        }
        isUngradedSurvey() {
          return "survey" === this.get("quiz_type");
        }
        publish_url() {
        ...
        url() {
          ...
          if (
            this.isQuizzesNext() &&
            null !== (t = ENV.PERMISSIONS) &&
            void 0 !== t &&
            t.manage &&
            null !== (e = ENV.FLAGS) &&
            void 0 !== e &&
            e.new_quizzes_modules_support
          )
            return this.edit_url();
          return `${this.get("base_url")}/${this.get("id")}`;
        }
        edit_url() {
          const t = this.isQuizzesNext() ? "?quiz_lti" : "";
          return `${this.get("base_url")}/${this.get("id")}/edit${t}`;
        }
        deletion_url() {
          if (this.isQuizzesNext())
            return `${this.get("base_url")}/${this.get("id")}`;
        ...
        initAllDates() {
          let t;
          if (null != (t = this.get("all_dates")))
            return this.set("all_dates", new h["a"](t));
        }
        publish() {
          this.set("published", true);
          return o.a.ajaxJSON(this.get("publish_url"), "POST", {
            quizzes: [this.get("id")],
          });
        }
        unpublish() {
          this.set("published", false);
          return o.a.ajaxJSON(this.get("unpublish_url"), "POST", {
            quizzes: [this.get("id")],
          });
        }
        disabledMessage() {
        ...
        destroy(t) {
          ...
          r.a.Model.prototype.destroy.call(this, e);
        }
        defaultDates() {
          return new d["a"]({
        ...
        duplicate(t) {
          const e = this.get("course_id");
          const s = this.get("id");
          return o.a.ajaxJSON(
            `/api/v1/courses/${e}/assignments/${s}/duplicate`,
            "POST",
            {
              quizzes: [s],
              result_type: "Quiz",
        ...
        duplicate_failed(t) {
          ...
          o.a.ajaxJSON(
            `/api/v1/courses/${i}/assignments/${n}/duplicate${u}`,
            "POST",
            {},
            t
          );
        ...
        retry_migration(t) {
          ...
          o.a.ajaxJSON(
            `/api/v1/courses/${e}/content_exports?export_type=quizzes2&quiz_id=${s}&include[]=migrated_quiz`,
            "POST",
            {},
            t
          );
        }
        pollUntilFinishedLoading(t) {
          this.isDuplicating() && this.pollUntilFinished(t, this.isDuplicating);
          this.isMigrating() && this.pollUntilFinished(t, this.isMigrating);
        }
        pollUntilFinished(t, e) {
          const s = this.get("course_id");
          const i = this.get("id");
          const n = new m["a"](t, 5 * t, (t) => {
            this.fetch({
              url: `/api/v1/courses/${s}/assignments/${i}?result_type=Quiz`,
            }).always(() => {
              t();
              if (!e()) return n.stop();
            });
        ...
        allDates() {
          const t = this.get("all_dates");
          const e = (t && t.models) || [];
          return a.a.map(e, (t) => t.toJSON());
        }
        singleSectionDueDate() {
          return (
            f(a.a.find(this.allDates(), "dueAt"), (t) =>
              t.dueAt.toISOString()
            ) || this.dueAt()
          );
        }
        isOnlyVisibleToOverrides(t) {
      ...
      z.prototype.resourceName = "quizzes";
...
//# sourceMappingURL=249-c-aa9ea380d0.js.map
