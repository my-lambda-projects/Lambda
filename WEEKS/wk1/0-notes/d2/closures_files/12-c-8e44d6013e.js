        ...
        if (!y[x] && (A || (x && !b[x]))) {
          ...
          if (!N) {
            ...
            for (var E = S.length; q < E; q++) {
              ...
              if (!F.match(v)) {
                var J = "";
                for (var L = 0, T = F.length; L < T; L++)
                  F.charCodeAt(L) > 127 ? (J += "x") : (J += F[L]);
                if (!J.match(v)) {
                  var W = S.slice(0, q);
                  var Y = S.slice(q + 1);
                  var $ = F.match(d);
