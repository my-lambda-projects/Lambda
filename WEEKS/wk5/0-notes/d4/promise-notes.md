...
```js
let mapAsync = require('async').map;

let urls = [ 'one.png', 'two.png' ];
...
```js
let Promise = require('bluebird');
let loadImageAsync = Promise.promisify(loadImage);
```
...
```js
let urls = ['one.png', 'two.png', 'three.png'];
let promises = urls.map(loadImageAsync);
...
```js
let thumbnail = Promise.resolve(defaultThumbnail);
...
```js
let Promise = require('bluebird')
let xhrAsync = Promise.promisify(require('xhr'))
```
